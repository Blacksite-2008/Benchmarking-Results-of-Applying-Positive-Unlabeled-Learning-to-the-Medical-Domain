...
from unbal_loss import OversampledPULossFunc, OversampledNNPULossFunc, NNZeroOneTrain, ZeroOneTest, NNsigmoidTrain, PUsigmoidTrain
from unbal_loss import Auc_loss, PUF1, PUPrecision, PURecall
from set_random_seed import set_seed

torch.backends.cudnn.enabled = False

def train_nnpu_sigmoid(num_initial_pos, learning_rate, weight_decay, num_batches, seed, dataset):
    epochs = 200
    bn_parameter = 0.9
    prior_prime = 0.5
    ...
    # For uPU sigmoid training
    criterion_training_upu = PUsigmoidTrain()
    # For nnPU sigmoid training
    criterion_training_nn = NNsigmoidTrain()
    # For 01 testing
    ...
    # Load data set
    xy_pos_train, xy_unlabel_train, xy_pos_test, xy_neg_test, prior = load_dataset(dataset, 60, 4074, seed)
    ...
    for epoch in range(epochs):
        ...
        for (train_positive, train_unlabeled) in zip(trainloader_positive, trainloader_unlabeled):
            ...
            loss_nn = criterion_training_nn(train_outputs_positive, train_outputs_unlabeled, prior)
            loss_upu = criterion_training_upu(train_outputs_positive, train_outputs_unlabeled, prior)
            ...
            if loss_nn > - 0.2:
                ...
                loss = - loss_nn
            #loss=criterion_training_nn(train_outputs_positive, train_outputs_unlabeled, prior)
            loss.backward()
            optimizer.step()
            #scheduler.step()
            ...
            training_nn_loss.append(one_iteration_running_loss)
            # Calculate various losses
            # objective
        training_nn_losses.append(np.mean(training_nn_loss))
        ...
        print("epoch: ",epoch)
        print("train_nn_losses: ",training_nn_losses)
        print("test_F1s: ",test_F1s)
        print("Auc_losses: ",Auc_losses)
        print("test_01_losses: ",test_01_losses)


        savename = '/home/Student/s4570079/prj_imbalanced_nnPU/result/nnpu_sigmoid_result_pos_%i_lr_%i_wd_%i_batches_%i_trial_%i_%s.mat' %(num_initial_pos, learning_rate, weight_decay, num_batches, seed, dataset)
        scio.savemat(savename,
                     {'prior': prior,
                      'running_losses': running_losses,
                      'training_nn_losses': training_nn_losses,

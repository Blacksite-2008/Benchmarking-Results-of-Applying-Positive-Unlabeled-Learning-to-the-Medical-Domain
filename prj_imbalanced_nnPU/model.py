...
class CNN(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv3 = nn.Conv2d(200, 200, kernel_size=3, stride=2, padding=1)
        self.bn3 = nn.BatchNorm2d(200)
        self.conv4 = nn.Conv2d(200, 400, kernel_size=3, padding=1)
        self.bn4 = nn.BatchNorm2d(400)
        self.conv5 = nn.Conv2d(400, 400, kernel_size=3, padding=1)
        self.bn5 = nn.BatchNorm2d(400)
        self.conv6 = nn.Conv2d(400, 400, kernel_size=3, stride=2, padding=1)
        self.bn6 = nn.BatchNorm2d(400)
        self.conv7 = nn.Conv2d(400, 400, kernel_size=3, padding=1)
        self.bn7 = nn.BatchNorm2d(400)
        self.conv8 = nn.Conv2d(400, 400, kernel_size=1)
        self.bn8 = nn.BatchNorm2d(400)
        self.conv9 = nn.Conv2d(400, 10, kernel_size=1)
    ...
    def forward(self, x):
        ...
        x = self.bn9(x)
        x = F.relu(x)
        #print("before view: ", np.shape(x))
        x = x.view(-1, 500)
        #print("after view: ", np.shape(x))
        #sys.exit()
        x = self.l1(x)
        x = F.relu(x)
        x = self.l2(x)

...
# training loss, sigmoid, PU
class PUsigmoidTrain(nn.Module):
    def __init__(self):
        super(PUsigmoidTrain, self).__init__()
        return

    def forward(self, output_p, output_n, prior):
        cost = prior * torch.mean(torch.sigmoid(-output_p))
        cost = cost + torch.mean(torch.sigmoid(output_n))
        cost = cost - prior * torch.mean(torch.sigmoid(output_p))
        return cost

# non-negative training loss, sigmoid
class NNsigmoidTrain(nn.Module):
    def __init__(self):
        super(NNsigmoidTrain, self).__init__()
        return

    def forward(self, output_p, output_n, prior):
        cost = torch.mean(torch.sigmoid(output_n)) - prior * torch.mean(torch.sigmoid(output_p))
        #cost = max(cost, 0)
        #cost = cost + prior * torch.mean(torch.sigmoid(-output_p))
        return cost

...
def process_args(arguments):
    ...
    #set random set
    parser.add_argument('--random_seed', '-seed',  type=int,
                        default=1, help='random_seed')
    #choose settings for each indiviual dataset
    parser.add_argument('--preset', '-p', type=str, default=None,
                        choices=['exp-mit', 'exp-aha', 'exp-st', 'exp-ptb'],
                        help="Preset of configuration\n"
                             "exp-mit: The setting of mit experiment in Experiment\n"
                             "exp-aha: The setting of aha experiment in Experiment\n"
                             "exp-st: The setting of st-t experiment in Experiment\n"
                             "exp-ptb: The setting of ptb experiment in Experiment")
    #choose dataset to use
    parser.add_argument('--dataset', '-d', default='mit', type=str, choices=['mit', 'aha','st','ptb'],
                        help='The dataset name')
    #choose the no. of labeled data
    parser.add_argument('--labeled', '-l', default=1000, type=int,
                        help='# of labeled data')
    #choose the no. of unlabeled data
    parser.add_argument('--unlabeled', '-u', default=69620, type=int,
                        help='# of unlabeled data')
    #set epoch
    parser.add_argument('--epoch', '-e', default=100, type=int,
                        help='# of epochs to learn')
    #beta parameter for nnPU
    parser.add_argument('--beta', '-B', default=0., type=float,
                        help='Beta parameter of nnPU')
    #gamma parameter for nnPU
    parser.add_argument('--gamma', '-G', default=1., type=float,
                        help='Gamma parameter of nnPU')
    #choose loss funciton, default sigmoid
    parser.add_argument('--loss', type=str, default="sigmoid", choices=['logistic', 'sigmoid'],
                        help='The name of a loss function')
    #what model to use, linear/CNN/MLP
    parser.add_argument('--model', '-m', default='cnn', choices=['linear', '3lp', 'mlp'],
                        help='The name of a classification model')
    #stepsize for optmization
    parser.add_argument('--stepsize', '-s', default=1e-5, type=float,
                        help='Stepsize of gradient method')
    #dir to print the output
    parser.add_argument('--out', '-o', default='/home/Student/s4570079/prj_nnPU/result/st_12',
                        help='Directory to output the result')
    args = parser.parse_args(arguments)
    if args.gpu >= 0 and chainer.backends.cuda.available:
        chainer.backends.cuda.get_device_from_id(args.gpu).use()
        chainer.backends.cuda.set_max_workspace_size(10732175360)
        #chainer.backends.cuda.cupy.cuda.set_allocator(None)
        #chainer.backends.cuda.cupy.cuda.set_pinned_memory_allocator(None)
        chainer.backends.cuda.cupy.random.seed(args.random_seed)
        chainer.config.cudnn_deterministic = True
    os.environ["CUPY_GPU_MEMORY_LIMIT"] = "10732175360"
    os.environ["CHAINER_SEED"] = str(args.random_seed)
    random.seed(args.random_seed)
    np.random.seed(args.random_seed)
    #chainer.config.cudnn_deterministic = True
    if args.preset == "exp-mit":
        args.labeled = 1000
        args.unlabeled = 69620
        args.dataset = "mit"
        args.batchsize = 500
        args.model = "cnn"
        args.stepsize = 1e-5
    elif args.preset == "exp-aha":
        args.labeled = 60
        args.unlabeled = 4074
        args.dataset = "aha"
        args.batchsize = 500
        args.model = "cnn"
        args.stepsize = 1e-4
    elif args.preset == "exp-st":
        args.labeled = 300
        args.unlabeled = 22797
        args.dataset = "st"
        args.batchsize = 500
        args.model = "cnn"
        args.stepsize = 1e-5
    elif args.preset == "exp-ptb":
        args.labeled = 300
        args.unlabeled = 19634
        args.dataset = "ptb"
        args.batchsize = 500
        args.model = "cnn"
        args.stepsize = 1e-5
    ...
    if args.dataset == "mit":
        assert (0 < args.unlabeled <= 69620)
    elif args.dataset == "aha":
        assert (0 < args.unlabeled <= 4074)
    elif args.dataset == "st":
        assert (0 < args.unlabeled <= 267960)
    else:
        assert (0 < args.unlabeled <= 19634)
    assert (0. <= args.beta)
...
class MultiPUEvaluator(chainer.training.extensions.Evaluator):
    ...
    def compute_summary(self, summary):
        ...
        for k, values in summary.items():
            ...
            print("summary: ",computed_summary[k])
